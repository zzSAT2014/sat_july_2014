...
def correctness_update(info,input):
	...
	if inputBool.upper() == 'TRUE': inputBool = True
	elif inputBool.upper() == 'FALSE': inputBool = False
		...
		def inner(dic):
			for index,line in enumerate(self.file):
				...
				if index == 0: continue
					...
					print line 
					name,date,serialNum, extraction, sentenceNum ,phrase, word  = line.split()
					...
					phrase = phrase.split(',')
					sentenceNum = sentenceNum.split(',')

					 
					word = word.split(',') 

					if phrase == ['None']: phrase = []
					if sentenceNum == ['None']: sentenceNum = []
					if word == ['None']: word = []
					...
					sentence_correctness = len(sentenceNum)#****************************************	update to real correctness rate when possible		  					
					...
					sub_dict(dic,[name,'paragraph','date',date,'sentence'],func = update_clause((sentence_correctness,sentenceNum)),value=()) #date specific : date:[(listNum,correctRate).....] appender
					#print dic
					sub_dict(dic,[name,'paragraph','date',date,'extraction'],func = update_correctness((extraction,serialNum)),value=()) #list specific : lisNum:[(date,correctRate).....] appender
					#print dic
					...
					for identi,cate in zip(['word','phrase'],[word,phrase]):    #word specific: word: wrong : (numdates,[....]), correct : (numdates,[....]), mostRecent : (flag,date)
						for atom in cate: #for word in each category
							#translate(word)
							sub_dict(dic,[name,'paragraph',identi,atom],func = word_add(date) ,value = (0,[]))

					
					sentence = (len(sentenceNum),sentenceNum)

					for identi,cate in zip(['extraction','sentence','date'],[extraction,sentence,date]):		
						sub_dict(dic,[name,'paragraph',serialNum,identi],value = cate)
		return inner
