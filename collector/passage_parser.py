		...
		def inner(dic):
			for index,line in enumerate(self.file):
				...
				if index == 0: continue
					...
					print line 
					name,date,serialNum, structure, attitude, group, phrase, word  = line.split()
					...
					group_correctness = len(group)#****************************************	update to real correctness rate when possible		  					
					...
					sub_dict(dic,[name,'passage','date',date,'group'],func = update_clause((group_correctness,group)),value=()) #date specific : date:[(listNum,correctRate).....] appender
					#print dic
					sub_dict(dic,[name,'passage','date',date,'attitude'],func = update_correctness((attitude,serialNum)),value=()) #list specific : lisNum:[(date,correctRate).....] appender
					#print dic
					sub_dict(dic,[name,'passage','date',date,'structure'],func = update_correctness((structure,serialNum)),value=())
					...
					for identi,cate in zip(['attitude','structure','group','date'],[attitude,structure,group,date]):		
